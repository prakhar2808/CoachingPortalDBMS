...
public class TeacherController {
	...
	public ModelAndView teacherHome() {
		...
		int teacherId = Integer.parseInt(userDetail.getUsername().substring(3));
		List<Teaches> teachesList = teachesService.getCourseAndBatch(teacherId);
		model.addObject("teachesList",teachesList);
		Teacher teacher = teacherService.getTeacherByTeacherId(teacherId);
		model.addObject("teacher",teacher);
		return model;
	}
	
	@RequestMapping("/teacher/viewStudents/{courseId}/{batchId}")
	public ModelAndView viewStudents(@PathVariable("courseId") String courseId,
	...
	public ModelAndView viewAttendance() {
		...
		UserDetails userDetail = (UserDetails) auth.getPrincipal();
		int teacherId = Integer.parseInt(userDetail.getUsername().substring(3));
		List<TeacherAttendance> attList = teacherAttendanceService.getAllAttendanceByTeacherId(teacherId);
